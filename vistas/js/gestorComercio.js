...
var codDivisa = $("#codigoDivisa").val();
...
if($("#codigoDivisa").val() != ""){
	...
	switch(codDivisa){
		...
		case "PEN":
			$("#seleccionarDivisa option:contains(NUEVO SOL)").attr("selected","true");
			//codDivisa = $("#codigoDivisa").attr("value",  ?);
			break;

		case "USD":
			$("#seleccionarDivisa option:contains(DOLAR)").attr("selected","true");
			//codDivisa = $("#codigoDivisa").attr("value",  ?);
			break;

		case "EUR":
			$("#seleccionarDivisa option:contains(EURO)").attr("selected","true");
			//codDivisa = $("#codigoDivisa").attr("value",  ?);
			break;	
...
var codDivisa = $("#seleccionarDivisa").val();
...
var publicKeyCulqi = $("#publicKeyCulqi").val();
var secretKeyCulqi = $("#secretKeyCulqi").val();
...
$("input[name='modoPaypal']").on("ifChecked",function(){
	...
	var modoCulqi = $("input[name='modoCulqi']:checked").val();

	$("#guardarInformacion").click(function(){

		cambiarInformacion(modoPaypal, modoPayu, modoCulqi);
...
$("input[name='modoCulqi']").on("ifChecked",function(){

	var modoCulqi = $(this).val();
	var modoPaypal = $("input[name='modoPaypal']:checked").val();
	var modoPayu = $("input[name='modoPayu']:checked").val();
...
$(".cambioInformacion").change(function(){
	...
	codDivisa = $("#seleccionarDivisa").val();
	...
	modoCulqi = $("input[name='modoCulqi']:checked").val();

	publicKeyCulqi = $("#publicKeyCulqi").val();

	secretKeyCulqi = $("#secretKeyCulqi").val();
...
function cambiarInformacion(modoPaypal, modoPayu, modoCulqi){

	var datos = new FormData();
	datos.append("divisa", codDivisa);
	...
	datos.append("modoCulqi", modoCulqi);	
	datos.append("publicKeyCulqi", publicKeyCulqi);
	datos.append("secretKeyCulqi", secretKeyCulqi);
